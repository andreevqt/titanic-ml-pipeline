...
RUN apt-get install -y libgomp1  # зависимость LightGBM
RUN useradd -m appuser         # non-root пользователь (безопасность)
RUN pip install --no-cache-dir # компактный слой зависимостей
USER appuser                   # запуск без root-прав
...
# Или готовый образ из GHCR
docker pull ghcr.io/andreevqt/titanic-ml-pipeline:latest
docker run --rm \
  -v $(pwd)/reports:/app/reports \
  -v $(pwd)/mlruns:/app/mlruns \
  ghcr.io/andreevqt/titanic-ml-pipeline:latest

# MLflow UI (после запуска pipeline)
...
gh repo create titanic-ml-pipeline --public
...
├── src/                   # модули: config, etl, train, evaluate, monitoring, pipeline
├── tests/                 # pytest-тесты (9 тестов)
├── reports/               # графики и HTML-отчёты (генерируются)
├── mlruns/                # MLflow эксперименты (генерируются)
├── Dockerfile
├── docker-compose.yml
